...
from task import BaseWaferTask, ListWaferTask, ComputeReticleTask, DramAccessReticleTask, PeerAccessReticleTask, FusedReticleTask
...
class LpReticleLevelWseEvaluator(BaseWseEvaluator):
    ...
    def __init__(self, 
        ...
        assert isinstance(task, ListWaferTask)
    ...
    def __build_annotated_graph(self) -> DiGraph:
        ...
        for reticle_task in self.task:
            assert isinstance(reticle_task, FusedReticleTask)
            ...
            for subtask in reticle_task.get_subtask_list():
                if subtask.task_type == 'compute':
                    ...
                    ndata['compute_mark'][vrid] = subtask.data_amount
    ...
    def __lp_solver(self, G: DiGraph) -> float:
        ...
        for edge, edata in G.edges(data=True):
            add_dram_access_constraint(edata)
